...
| 14 | 接受一起洗澡 | `accept_bathing_together` | 允许一起洗澡 |
| 15 | 接受玩家按摩 | `accept_player_massage` | 允许玩家为角色按摩 |
| 16 | 给角色按摩 | `accept_character_massage` | 允许角色为玩家按摩 |
...
export const UNLOCK_STATUS_NAMES: Record<keyof CharacterUnlocks, string> = {
    ...
    accept_bathing_together: '一起洗澡',
    accept_player_massage: '接受玩家按摩',
    accept_character_massage: '给角色按摩'
};
```
...
1. **检查解锁状态**: 当玩家提出以下类型的请求时，检查对应状态是否已解锁：
   ...
   - 一起洗澡 → accept_bathing_together
   - 玩家按摩角色 → accept_player_massage
   - 角色按摩玩家 → accept_character_massage
...
export const UNLOCK_AFFINITY_REQUIREMENTS: Record<keyof CharacterUnlocks, number> = {
    ...
    accept_bathing_together: 45,
    accept_player_massage: 35,
    accept_character_massage: 30
};
