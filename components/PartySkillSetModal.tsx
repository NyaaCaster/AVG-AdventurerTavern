...
import { CharacterSkillsData, getAllCharacterSkills, updateCharacterSkills } from '../services/db';
...
const CharacterSkillPanel: React.FC<{
  ...
  characterSkills: CharacterSkillsData;
  userName: string;
  onUpdateSkills: (skills: CharacterSkillsData) => void;
  onPrev?: () => void;
  onNext?: () => void;
  hasPrev?: boolean;
  hasNext?: boolean;
  ...
  const handleSelectSkill = useCallback((skillId: number | null) => {
    ...
    const newSkills: CharacterSkillsData = {
      ...characterSkills,
...
const PartySkillSetModal: React.FC<PartySkillSetModalProps> = ({
  ...
  const [allCharacterSkills, setAllCharacterSkills] = useState<Record<string, CharacterSkillsData>>({});
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    if (isOpen && userId && slotId) {
      setIsLoading(true);
      getAllCharacterSkills(userId, slotId)
        .then(data => {
          setAllCharacterSkills(data);
        })
        .finally(() => {
          setIsLoading(false);
        });
    }
  }, [isOpen, userId, slotId]);
  ...
  const handleUpdateSkills = useCallback((characterId: string, skills: CharacterSkillsData) => {
    setAllCharacterSkills(prev => ({
      ...prev,
      [characterId]: skills
    }));
  ...
  const getCharacterSkillsData = (characterId: string): CharacterSkillsData => {
    return allCharacterSkills[characterId] || {
      characterId,
      slot1: null,
      slot2: null,
      slot3: null,
              ...
              <div className="h-full p-2 md:p-4 overflow-y-auto custom-scrollbar">
                {selectedCharacterId === null ? (
                  ...
                  <CharacterSkillPanel
                    ...
                    userName={userName}
                    onUpdateSkills={(skills) => handleUpdateSkills(selectedCharacterId, skills)}
                    onPrev={handlePrev}
                    onNext={handleNext}
