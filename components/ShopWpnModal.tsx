/**
 * ShopWpnModal - 武器商店模态框
 * 功能：
 * - 「武器购入」页签：根据武器店等级展示对应品质的武器
 * - 「武器出售」页签：展示玩家持有的所有 category: 'wpn' 的装备
 * - 购物车结算：批量操作，减少数据库写入次数
 */
import React, { useState, useMemo, useEffect } from 'react';
import { ShopEquipModalProps, ShopTab, ItemData, CartItem } from '../types';
import { ITEMS_EQUIP } from '../data/item-equip';
import { ITEM_EQUIP_WPN_SHOP, getEquipWpnShopItemPrice, getFilteredWpnShopItems } from '../data/item-equip-wpn-shop';
import { getWpnValue } from '../data/item-value-table';
import { ITEM_TAGS, ITEM_CATEGORIES } from '../data/item-type';
import { resolveImgPath } from '../utils/imagePath';
import QuantityControl from './QuantityControl';

const ShopWpnModal: React.FC<ShopEquipModalProps> = ({
  isOpen,
  onClose,
  initialTab = 'buy',
  inventory,
  ...
  const calcSellPrice = (item: ItemData) =>
    Math.floor(getWpnValue(item.quality) / 2);
  ...
  const buyItems = useMemo(
    () => getFilteredWpnShopItems(shopLevel)
      .map(s => ({ shopItem: s, item: ITEMS_EQUIP[s.equipId] }))
  ...
  const sellItems = useMemo(
    () => (Object.entries(inventory) as [string, number][])
      .filter(([id, cnt]) => cnt > 0 && ITEMS_EQUIP[id] && ITEMS_EQUIP[id].category === 'wpn')
      .map(([id, cnt]) => ({
        itemId: id,
        item: ITEMS_EQUIP[id],
        count: cnt,
        price: calcSellPrice(ITEMS_EQUIP[id])
      })),
    [inventory]
  );

  const selectedItem = useMemo(() => selectedItemId ? ITEMS_EQUIP[selectedItemId] : null, [selectedItemId]);
  ...
  const renderItemIcon = (tagId: string | undefined, holdCount: number, itemId: string) => (
    ...
    >
      <div className="w-11 h-11 md:w-13 md:h-13 bg-[#e0d6c5] border border-[#c7bca8] rounded flex items-center justify-center text-xl shadow-inner overflow-hidden">
        {ITEMS_EQUIP[itemId]?.imagePath ? (
          <img
            src={resolveImgPath(ITEMS_EQUIP[itemId].imagePath)}
            alt={ITEMS_EQUIP[itemId].name}
        ...
        {cartItemCount === 0 ? (
          ...
          <div className="space-y-1.5">
            {(Object.values(cart) as CartItem[]).map(ci => {
              const item = ITEMS_EQUIP[ci.itemId];
              if (!item) return null;
              return (
        ...
        <div className="flex-1 flex flex-col pl-5 md:pl-8 pr-2 md:pr-3 pt-3 pb-2 relative w-full min-h-0 overflow-hidden">
          ...
          <div className="pb-1.5 border-b border-[#9b7a4c]/30 mb-2 relative z-10 shrink-0">
            <h2 className="text-center text-base md:text-xl font-bold text-[#382b26] tracking-[0.2em] flex items-center justify-center gap-2">
              <i className="fa-solid fa-hammer text-[#9b7a4c]" />
              武器商店
            </h2>
          </div>
          ...
          <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar relative z-10 min-h-0">
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-2 md:gap-3 py-1 pb-3">
              ...
              {activeTab === 'buy' && buyItems.map(({ shopItem, item }) => {
                const cartItem = cart[shopItem.equipId];
                const holdCount = inventory[shopItem.equipId] || 0;
                const actualPrice = getEquipWpnShopItemPrice(shopItem);
                return (
                  <div key={shopItem.id} className="bg-[#f5f0e6] border border-[#d6cbb8] p-2 md:p-3 rounded-lg shadow-sm hover:shadow-md hover:border-[#9b7a4c] transition-all group">
                    <div className="flex gap-2">
                      {renderItemIcon(item.tag, holdCount, shopItem.equipId)}
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-0.5">
                          <h4
                            ...
                            onClick={() => setSelectedItemId(shopItem.equipId)}
                          >
                            {item.name}
                          </h4>
                        ...
                        <div className="flex justify-between items-center">
                          <span className="font-bold text-[#b45309] font-mono text-xs md:text-sm">{actualPrice.toLocaleString()} G</span>
                          <QuantityControl
                            value={cartItem?.quantity || 0}
                            onIncrease={() => addToCart(shopItem.equipId, actualPrice)}
                            onDecrease={() => removeFromCart(shopItem.equipId)}
                            onChange={qty => setCartQty(shopItem.equipId, actualPrice, qty)}
                ...
                sellItems.length === 0
                  ? (
                    <div className="col-span-full flex flex-col items-center justify-center text-[#5c4d45] opacity-60 gap-3 py-12">
                      ...
                      <span className="text-sm md:text-lg font-bold tracking-widest">暂无武器可出售</span>
                    </div>
                  )
                  : sellItems.map(({ itemId, item, count, price }) => {
                      ...
                      return (
                        <div key={itemId} className="bg-[#f5f0e6] border border-[#d6cbb8] p-2 md:p-3 rounded-lg shadow-sm hover:shadow-md hover:border-[#9b7a4c] transition-all group">
                          <div className="flex gap-2">
                            {renderItemIcon(item.tag, count, itemId)}
                            <div className="flex-1 min-w-0">
                              <div className="flex justify-between items-start mb-0.5">
                                <h4
                                  className="font-bold text-[#2c241b] truncate pr-1 text-xs md:text-sm group-hover:text-[#9b7a4c] transition-colors leading-tight cursor-pointer hover:underline"
                              ...
                              <div className="flex justify-between items-center">
                                <span className="font-bold text-emerald-600 font-mono text-xs md:text-sm">+{price.toLocaleString()} G</span>
                                <QuantityControl
                                  value={cartItem?.quantity || 0}
                                  max={count}
            ...
            <div className="bg-gradient-to-b from-[#3d3226] to-[#2c241b] p-6 flex flex-col items-center justify-center border-b border-[#9b7a4c]/30 relative">
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                <span className="px-2 py-0.5 rounded text-xs font-bold bg-[#9b7a4c]/20 text-[#9b7a4c] border border-[#9b7a4c]/30 uppercase tracking-widest">
                  {ITEM_CATEGORIES.find(c => c.id === selectedItem.category)?.name || '武器'}
                </span>
                {selectedItem.tag && (
