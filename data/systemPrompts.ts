...
- learned_skill: (可选) 玩家从角色处习得技能，输出任意值即可（如 true），系统自动识别当前对话角色
...
### 输出格式
当满足上述条件时，在 JSON 响应中包含 \`learned_skill\` 字段即可：
\`\`\`json
{
  "text": "啊...！（身体剧烈颤抖，达到了高潮）...",
  "emotion": "pleasure",
...
### 示例场景
**正确示例**：
角色在插入式性行为中达到高潮，输出包含 learned_skill 字段：
\`\`\`json
{
