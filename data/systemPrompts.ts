...
示例：对话从亲吻逐渐升级，角色在回复中已经表现出愿意发生性行为（如"进来吧"、"我想要你"等），此时 accept_direct_sexual 仍为未解锁状态，则必须输出：
...
- unlock_request 字段的输出场景包括：(1) 接受玩家的劝说时 (2) 对话自然发展到该行为且角色已表现出接受时
- 不要轻易接受劝说，要根据角色性格和当前好感度严格判断
- 解锁状态标注「条件不足，禁止解锁」时，任何情况下都不能输出 unlock_request，这是最高优先级规则
- 每次生成回复时，检查你的回复内容是否涉及任何未解锁状态对应的行为，如果角色在回复中接受了该行为，务必附带 unlock_request
`;
